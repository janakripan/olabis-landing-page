      ...
      <div className="max-w-7xl mx-auto px-6">
        ...
        <div className="text-center mb-16">
          ...
          <motion.div 
            ...
            className="flex justify-center items-center gap-1 text-accent"
          >
            {[...Array(5)].map((_, i) => <FaStar key={i} />)}
            <span className="text-text-primary font-bold ml-2">4.9/5 Average Rating</span>
          </motion.div>
        </div>
        ...
        <div className="grid grid-cols-1 md:grid-cols-3 gap-8">
          {testimonials.map((test, index) => (
            <motion.div
              ...
              className="group relative bg-surface border border-white/5 rounded-[16px] p-8 flex flex-col hover:-translate-y-1 hover:shadow-[var(--shadow-card)] hover:border-accent/40 transition-all duration-500 overflow-hidden"
            >
              {/* Premium Hover Glow Effect */}
              <div className="absolute inset-0 bg-gradient-to-br from-accent/0 to-accent/0 group-hover:from-accent/5 group-hover:to-transparent transition-all duration-500 z-0 pointer-events-none"></div>

              <div className="relative z-10 flex flex-col h-full">
                <div className="text-accent/50 mb-6 transition-transform group-hover:scale-110 group-hover:text-accent duration-500 origin-left">
                ...
                <p className="text-text-secondary leading-relaxed mb-8 flex-1 italic group-hover:text-text-primary transition-colors duration-500">
                  "{test.quote}"
                </p>
                
                <div className="flex items-center gap-4 mt-auto">
                  <div className={`w-12 h-12 rounded-full ${test.color} flex items-center justify-center font-bold text-lg ring-2 ring-transparent group-hover:ring-white/10 transition-all`}>
                    {test.initials}
                  </div>
                  <div>
                    <div className="font-bold text-text-primary">{test.author}</div>
                    <div className="text-xs text-text-secondary">{test.role}</div>
                  </div>
                </div>
              </div>
            </motion.div>
